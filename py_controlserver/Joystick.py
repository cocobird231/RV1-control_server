...
class Joystick(object):
    ...
    def init(self, hatMotionCallbackUp=None, hatMotionCallbackDown=None,hatMotionCallbackLeft=None,hatMotionCallbackRight=None, buttonACallback=None, buttonBCallback=None, buttonXCallback=None, buttonYCallback=None):
        pygame.init()
        ...
        self.hatMotionCallbackUp = hatMotionCallbackUp
        self.hatMotionCallbackDown = hatMotionCallbackDown
        self.hatMotionCallbackLeft = hatMotionCallbackLeft
        self.hatMotionCallbackRight = hatMotionCallbackRight
        self.buttonACallback = buttonACallback
        self.buttonBCallback = buttonBCallback
        self.buttonXCallback = buttonXCallback
        self.buttonYCallback = buttonYCallback
        ...
        while True:
            ...
            for event in [pygame.event.wait(), ] + pygame.event.get():
                ...
                if event.type == QUIT:
                    ...
                    if(event.value[1] == 1):
                        if self.hatMotionCallbackUp:
                            self.hatMotionCallbackUp()
                    elif(event.value[1] == -1):
                        if self.hatMotionCallbackDown:
                            self.hatMotionCallbackDown()
                    elif(event.value[0] == -1):
                        if self.hatMotionCallbackLeft:
                            self.hatMotionCallbackLeft()
                    elif(event.value[0] == 1):
                        if self.hatMotionCallbackRight:
                            self.hatMotionCallbackRight()
                elif event.type == JOYBUTTONUP:
                    self.joy[event.joy].button[event.button] = 0
                elif event.type == JOYBUTTONDOWN:
                    self.joy[event.joy].button[event.button] = 1
                    if event.button == 0:  #'A'
                        if self.buttonACallback:
                            self.buttonACallback()
                    if event.button == 1:  #'B'
                        if self.buttonBCallback:
                            self.buttonBCallback()
                    if event.button == 2:  #'X'
                        if self.buttonXCallback:
                            self.buttonXCallback()
                    if event.button == 3:  #'Y'
                        if self.buttonYCallback:
                            self.buttonYCallback()
